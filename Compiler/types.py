...
class sint(_secret, _int):
    ...
    def receive_from_client(cls, n, client_id, message_type=ClientMessageType.NoType):
        ...
        """
        if program.active:
            # send shares of a triple to clients
    ...
    @vectorized_classmethod
    def receive_from_client_MAX(cls,m, n, client_id, message_type=ClientMessageType.NoType):
        triples_matrix = sint.Matrix(m, n)
        active = program.active
        if active:
            triples = list(itertools.chain(*(sint.get_random_triple() for i in range(m * n))))
            for i in range(m):
                for j in range(n):
                    triples_matrix[i][j] = -triples[3 * (i * n + j)]
        else:
            triples = [sint.get_random() for i in range(m * n)]
            for i in range(m):
                for j in range(n):
                    triples_matrix[i][j] = -triples[i * n + j]

        sint.write_shares_to_socket(client_id, triples, message_type)

        received = util.tuplify(cint.read_from_socket(client_id, m * n))

        received_matrix = cint.Matrix(m, n)
        for i in range(m):
            for j in range(n):
                received_matrix[i][j] = received[i * n + j]


        return received_matrix, triples_matrix, active

    @vectorized_classmethod
    def receive_from_client_AND(cls, n, client_id, message_type=ClientMessageType.NoType):
        triples_array = sint.Array(n)
        active = program.active
        if active:
            triples = list(itertools.chain(*(sint.get_random_triple() for i in range(n))))
            for i in range(n):
                triples_array[i] = -triples[3 * i]
        else:
            triples = [sint.get_random() for i in range(n)]
            for i in range(n):
                triples_array[i]= -triples[i]

        sint.write_shares_to_socket(client_id, triples, message_type)

        received = util.tuplify(cint.read_from_socket(client_id, n))

        received_array = cint.Array(n)
        for i in range(n):
            received_array[i] = received[i]
        return received_array, triples_array
    
    @vectorized_classmethod
    def receive_from_client_Single(cls, m, n, client_id, message_type=ClientMessageType.NoType):
        
        r_matrix = sint.Matrix(m, n)
        R_list = [0] * m                          
        for i in range(m):
            for j in range(n):
                tmp = sint.get_random_bit()
                r_matrix[i][j] = 2 * tmp - 1
                R_list[i] += 2 ** j * tmp
        triples = []

        if program.active:
            for i in range(m):
                triple2 = list(sint.get_random_square())
                r2 = R_list[i] * triple2[0]
                triple2.extend(R_list[i])
                triple2.extend(r2)
                triples.extend(triple2)
        else:
            for i in range(m):
                triples.extend(R_list[i])
        
        sint.write_shares_to_socket(client_id, triples, message_type)

        received = util.tuplify(cint.read_from_socket(client_id, m))

        Y_array = cint.Array(m)
        flag_list = []
        for i in range(m):
            Y_array[i] = received[i]
        return Y_array, r_matrix

    @vectorized_classmethod
    def receive_from_client_Multiple(cls, m, n, client_id, message_type=ClientMessageType.NoType):

        r_matrix = sint.Matrix(m, n)
        y_matrix = cint.Matrix(m, n)         
        R_list = [0] * m                          
        for i in range(m):
            for j in range(n):
                tmp = sint.get_random_bit()
        ...
        if program.active:
            for i in range(m):
                r2 = sint.get_random()
                triples.extend([R_list[i], r2, R_list[i] * r2])
        else:
            for i in range(m):
                triples.extend(R_list[i])
